...
def login_for_access_token(form_data: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
    if auth.password_too_long(form_data.password):
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
...
def analyze_image(
    ...
    for res in analysis_results:
        ...
        if not ab:
            # Fallback names like Disk_1 can repeat; reuse existing row if present.
            existing_ab = db.query(models.Antibiotic).filter(
                models.Antibiotic.name == medicine_name,
                models.Antibiotic.concentration_ug == 0
            ).first()
            ...
            if existing_ab:
                ...
                try:
                    ...
                    # Handle race/duplicate safely by re-querying existing row.
                    db.rollback()
    ...
    # Return plate with nested antibiotic loaded for immediate frontend display.
