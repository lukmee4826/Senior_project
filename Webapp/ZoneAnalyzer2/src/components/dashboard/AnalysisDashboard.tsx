    ...
    const processFiles = async () => {
      for (let i = 0; i < totalFiles; i++) {
        ...
        try {
          ...
          if (!response.ok) {
            throw new Error(`Error analyzing ${file.name}`);
          }
          ...
          // Handle error (maybe push an error result or alert)
        }
          ...
          {uploadedFiles.length > 0 && (
            <div className="mt-6 space-y-3">
                          ...
                          <CommandList>
                            ...
                            <CommandGroup>
                              {bacteriaList.map((bacteria) => (
                                ...
                                >
                                  <Check
                                    className={`mr-2 h-4 w-4 ${groupBacteria === bacteria.value
                                      ? "opacity-100"
                                      : "opacity-0"
                                      }`}
                                  />
              ...
              {uploadedFiles.map((file, idx) => (
                <div
                  key={idx}
                  className={`flex items-center gap-3 p-3 rounded-lg border ${selectedFiles.includes(idx)
                    ? isDark
                    ...
                    >
                      <PopoverTrigger asChild>
                        <Button
                          ...
                          className={`w-full mt-2 justify-between text-left ${isDark
                            ? "bg-gray-800 border-gray-700 text-gray-300 hover:bg-gray-750"
                            : "bg-white border-gray-300 text-gray-700 hover:bg-gray-50"
                            }`}
                        >
                          ...
                          <CommandList>
                            ...
                            <CommandGroup>
                              {bacteriaList.map((bacteria) => (
                                ...
                                >
                                  <Check
                                    className={`mr-2 h-4 w-4 ${bacteriaSelections[idx] ===
                                      bacteria.value
                                      ? "opacity-100"
                                      : "opacity-0"
                                      }`}
